...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/10 04:27:50 by rkenji-s          #+#    #+#             */
/*   Updated: 2021/10/10 04:27:50 by rkenji-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*conditionals_2(t_flags *info, char *str, int x2)
{
	char	*temp;

	if (info->hashtag == 1 && x2 != 0)
	{
		write (1, "0X", 2);
		info->width -= 2;
		info->size += 2;
...
int	print_x2(t_flags *info)
{
	...
	x2 = va_arg (info->arg, unsigned int);
	str = convert_hex2(x2);
	if (info->precision == 0 && info->point != 0 && x2 == 0)
	{
		conditionals_1(info, str);
		return (info->size);
	}
	str = conditionals_2(info, str, x2);
